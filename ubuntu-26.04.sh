...
tmpdir=$(pwd)/build/ubuntu-26.04

if [[ ! -d $tmpdir ]]; then
    mkdir -p $tmpdir
    debootstrap --arch=amd64 --variant=minbase resolute $tmpdir http://archive.ubuntu.com/ubuntu/
fi
...
tar --numeric-owner -cpf ubuntu-26.04-amd64-matt.tar -C $tmpdir .
xz -T0 ubuntu-26.04-amd64-matt.tar