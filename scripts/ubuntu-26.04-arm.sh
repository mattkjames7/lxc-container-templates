...
BUILD_DIR=$BUILD_DIR/ubuntu-26.04-arm

OUTPUT_DIR=${2:-$(pwd)/output}

if [[ ! -d $BUILD_DIR ]]; then
    mkdir -p $BUILD_DIR
    debootstrap --arch=arm64 --variant=minbase resolute $BUILD_DIR http://ports.ubuntu.com/ noble
fi

echo "ubuntu-26.04" > $BUILD_DIR/etc/hostname
echo "root:root" | chroot $BUILD_DIR chpasswd

chroot "$BUILD_DIR" apt update
chroot "$BUILD_DIR" apt install -y systemd systemd-sysv sudo iproute2
...
filename="${OUTPUT_DIR}/ubuntu-26.04-arm64-$(date +%Y%m%d%H%M%S).tar"
