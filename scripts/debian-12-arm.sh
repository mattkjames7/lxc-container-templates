...
BUILD_DIR=$BUILD_DIR/debian-12-arm

OUTPUT_DIR=${2:-$(pwd)/output}

if [[ ! -d $BUILD_DIR ]]; then
    mkdir -p $BUILD_DIR
    debootstrap --arch=arm64 --variant=minbase bookworm $BUILD_DIR http://deb.debian.org/debian/
fi

echo "debian-12" > $BUILD_DIR/etc/hostname
...
filename="${OUTPUT_DIR}/debian-12-arm64-$(date +%Y%m%d%H%M%S).tar"
