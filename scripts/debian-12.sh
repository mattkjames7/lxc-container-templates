...
BUILD_DIR=$BUILD_DIR/debian-12

OUTPUT_DIR=${2:-$(pwd)/output}

if [[ ! -d $BUILD_DIR ]]; then
    mkdir -p $BUILD_DIR
    debootstrap --arch=amd64 --variant=minbase bookworm $BUILD_DIR http://deb.debian.org/debian/
fi
...
filename="${OUTPUT_DIR}/debian-12-amd64-$(date +%Y%m%d%H%M%S).tar"
